...
async function refund(jobId:string,reason:string,message:string){
  ...
  try{
    ...
    for(const [bucket,amount] of Object.entries({promo,subscription,purchased})) if(Number(amount)>0) await client.query('insert into credit_ledger(user_id,bucket,delta,reason,reference_type,reference_id,idempotency_key) values($1,$2,$3,$4,$5,$6,$7) on conflict do nothing',[row.user_id,bucket,Number(amount),'generation_failed_auto_refund','generation',jobId,`vercel-refund:${jobId}:${bucket}`]);
...
export default async function handler(req:VercelRequest,res:VercelResponse){
  ...
  const userId=String(b.userId??'').trim(), type=b.type, prompt=String(b.prompt??'').trim();
  const quality:Quality=b.quality??'fast', seconds=Number.isInteger(b.seconds)?Number(b.seconds):0, audio=Boolean(b.audio), aspectRatio=b.aspectRatio??'9:16', draft=Boolean(b.draft);
  ...
  try{
    ...
    await client.query('commit');
  }catch(e){await client.query('rollback');console.error('[api/generations] reserve failed',String(e).slice(0,700));return res.status(500).json({error:'generation_create_failed'});}finally{client.release();}
  ...
  try{
    ...
    }else await pool.query('update generation_jobs set provider_job_id=$2 where id=$1',[id,started.providerJobId]);
  }catch(e){const message=String(e);console.error('[api/generations] provider submit failed',message.slice(0,700));await refund(id,'provider_submit_failed',message);return res.status(502).json({error:'provider_submit_failed',jobId:id,status:'refunded'});}
