...
async function refund(jobId: string, reason: string, message: string) {
  ...
  try {
    ...
    for (const [bucket,amount] of Object.entries({promo,subscription,purchased})) {
      if (Number(amount) > 0) await client.query('insert into credit_ledger(user_id,bucket,delta,reason,reference_type,reference_id,idempotency_key) values($1,$2,$3,$4,$5,$6,$7) on conflict do nothing',[row.user_id,bucket,Number(amount),'generation_failed_auto_refund','generation',jobId,`status-refund:${jobId}:${bucket}`]);
    }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!authorized(req,job.userId)) return res.status(401).json({error:'unauthorized'});
  ...
  if (job.status === 'processing' && job.providerJobId) {
    try {
      ...
      if (state.status === 'completed') {
        ...
        await refund(id,'provider_failed',state.error || 'provider_failed');
      }
    } catch (e) {
      const age = await pool.query('select extract(epoch from (now()-started_at))::int as age from generation_jobs where id=$1',[id]);
      if (Number(age.rows[0]?.age ?? 0) > 1200) await refund(id,'provider_timeout',String(e));
      else console.warn('[api/generation-status] provider poll error',String(e).slice(0,500));
    }
