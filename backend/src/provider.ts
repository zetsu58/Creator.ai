...
async function blobToDataUrl(blob:Blob,mime:string){const b=new Uint8Array(await blob.arrayBuffer());if(!b.length)throw new Error('huggingface_empty_output');return`data:${blob.type||mime};base64,${Buffer.from(b).toString('base64')}`;}
async function huggingFaceRequest(j:GenerationJob):Promise<ProviderStart>{const token=hfToken();if(!token)throw new Error('hf_token_missing');const client=new InferenceClient(token);const isVideo=j.type==='video'||j.type==='product_ad';if(isVideo){const out=await client.textToVideo({model:hfModelFor(j),inputs:j.prompt,provider:hfProvider() as any});return{providerJobId:`hf:${j.id}`,status:'completed',outputUrl:await blobToDataUrl(out,'video/mp4')}}const out=await client.textToImage({model:hfModelFor(j),inputs:j.prompt,provider:hfProvider() as any});return{providerJobId:`hf:${j.id}`,status:'completed',outputUrl:await blobToDataUrl(out,'image/png')}}
