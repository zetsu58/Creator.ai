...
export function omniRouteConfigured() {
...
function mediaUrl(value: any): string|null {
  ...
  if (typeof value === 'string') {
    const v = value.trim();
    return /^https?:\/\//i.test(v) || /^data:/i.test(v) ? v : null;
  }
  ...
  if (typeof value === 'object') {
    for (const key of ['url','video_url','image_url','output','data','result','file']) {
      const found = mediaUrl(value[key]);
      if (found) return found;
    }
  }
  return null;
...
async function request(path: string, init?: RequestInit, timeoutMs = 30_000) {
  const base = baseUrl();
  const key = apiKey();
  if (!base || !key) throw new Error('omniroute_not_configured');
  ...
  try {
    ...
    if (!res.ok) {
      const msg = body?.error?.message || body?.message || body?.error || raw;
      throw new Error(`omniroute_http_${res.status}:${String(msg).slice(0,1600)}`);
    }
    return body;
  } finally {
    clearTimeout(timer);
...
function imageSize(aspect?: string|null) {
...
function videoSize(aspect?: string|null) {
  return (aspect || '9:16').trim() === '16:9' ? '1280x720' : '720x1280';
}
...
export async function omniRouteStart(job: OmniRouteMediaJob): Promise<OmniRouteStart> {
  ...
  if (isVideo) {
    const model = process.env.OMNIROUTE_VIDEO_MODEL?.trim();
    if (!model) throw new Error('omniroute_video_model_missing');
    ...
    const body: any = {
      ...
      size: videoSize(job.aspectRatio),
    };
    if (job.imageUrl?.trim()) {
      body.input = [
        {type: 'text', text: job.prompt.trim()},
        {type: 'image', image: job.imageUrl.trim()},
      ];
    }
    const out = await request('/v1/videos/generations', {
      method: 'POST',
      headers: {'Idempotency-Key': job.id},
      body: JSON.stringify(body),
    }, 60_000);
    const status = String(out?.status || '').toLowerCase();
    const url = mediaUrl(out);
    if ((status === 'completed' || status === 'succeeded' || status === 'success') && url) {
      return {providerJobId: `omniroute:${String(out?.id || job.id)}`, status: 'completed', outputUrl: url};
    }
    const id = String(out?.id || out?.task_id || out?.video_id || '').trim();
    if (!id) throw new Error(`omniroute_missing_video_id:${JSON.stringify(out).slice(0,1200)}`);
    return {providerJobId: `omniroute:${id}`, status: 'processing'};
  }

  const model = process.env.OMNIROUTE_IMAGE_MODEL?.trim();
  if (!model) throw new Error('omniroute_image_model_missing');
  const out = await request('/v1/images/generations', {
    method: 'POST',
    headers: {'Idempotency-Key': job.id},
    body: JSON.stringify({
      model,
      prompt: job.prompt.trim(),
      n: 1,
      size: imageSize(job.aspectRatio),
      response_format: 'url',
    }),
  }, 120_000);
  const url = mediaUrl(out);
  if (!url) throw new Error(`omniroute_missing_image_url:${JSON.stringify(out).slice(0,1200)}`);
...
export async function omniRoutePoll(providerJobId: string): Promise<OmniRoutePoll> {
  ...
  const out = await request(`/v1/videos/${encodeURIComponent(id)}`, {method: 'GET'}, 30_000);
  const status = String(out?.status || '').toLowerCase();
  if (status === 'completed' || status === 'succeeded' || status === 'success') {
    const url = mediaUrl(out);
    return url ? {status: 'completed', outputUrl: url} : {status: 'failed', error: 'omniroute_completed_without_output'};
  }
  if (status === 'failed' || status === 'canceled' || status === 'cancelled' || status === 'expired') {
    return {status: 'failed', error: String(out?.error?.message || out?.error || status)};
  }
