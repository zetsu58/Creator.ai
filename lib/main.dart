...
class _CreatorHomeState extends State<CreatorHome> {
  ...
  Future<void> _refreshBackend() async {
    ...
    if (online) {
      try { nextCredits = await api.walletCredits(userId); } catch (_) {}
    }
    if (!mounted) return;
    setState(() { backendOnline = online; credits = nextCredits; checkingBackend = false; });
  }

  Future<void> _openTool(ToolSpec tool) async {
    final result = await Navigator.of(context).push<Map<String, dynamic>>(
      MaterialPageRoute(builder: (_) => ToolStudioPage(tool: tool, userId: userId, credits: credits, api: api)),
    );
    if (result != null) {
  ...
  Widget build(BuildContext context) {
    final pages = <Widget>[
      StudioHome(credits: credits, backendOnline: backendOnline, checkingBackend: checkingBackend, onRefresh: _refreshBackend, onOpenTool: _openTool),
...
class StudioHome extends StatelessWidget {
  const StudioHome({super.key, required this.credits, required this.backendOnline, required this.checkingBackend, required this.onRefresh, required this.onOpenTool});
  final int credits;
  ...
  Widget build(BuildContext context) => LayoutBuilder(builder: (context, c) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          Row(children: [
            const VeyraMark(), const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Veyra AI', style: TextStyle(fontSize: 25, fontWeight: FontWeight.w900)), Text(vt(context, 'tagline'), style: const TextStyle(color: Colors.white54, fontSize: 12))])),
          ...
          GridView.count(
            shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: columns,
            childAspectRatio: columns >= 3 ? 1.35 : 1.08, crossAxisSpacing: 12, mainAxisSpacing: 12,
            children: tools.map((tool) => _ToolCard(tool: tool, onTap: () => onOpenTool(tool))).toList(),
          ),
          const SizedBox(height: 28), const _TrustStrip(),
        ],
      ),
    );
  });
...
class _BackendBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => InkWell(
    ...
    child: Container(
      ...
      child: Row(children: [
        if (loading) const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) else Icon(online ? Icons.cloud_done_outlined : Icons.cloud_off_outlined, size: 20, color: online ? Colors.greenAccent : Colors.orangeAccent),
        const SizedBox(width: 9),
        Expanded(child: Text(loading ? vt(context, 'cloud_check') : online ? vt(context, 'cloud_on') : vt(context, 'cloud_off'), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700))),
...
class _HeroCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
    ...
    child: InkWell(
      onTap: onTap,
      child: Container(
        constraints: const BoxConstraints(minHeight: 235), padding: const EdgeInsets.all(24),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 9), Text(vt(context, 'hero_sub')),
          const SizedBox(height: 22), Row(children: [const Icon(Icons.auto_awesome, color: CreatorTheme.cyan), const SizedBox(width: 9), Text(vt(context, 'open_video'), style: const TextStyle(fontWeight: FontWeight.w900)), const SizedBox(width: 8), const Icon(Icons.arrow_forward)]),
        ]),
      ),
    ),
  );
...
class _ProBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      const Icon(Icons.workspace_premium_outlined, color: CreatorTheme.gold, size: 32), const SizedBox(width: 13),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Veyra Pro', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 17)), const SizedBox(height: 3), Text(vt(context, 'pro_sub'), style: const TextStyle(color: Colors.white60, fontSize: 12))])),
    ...
    child: InkWell(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(vt(context, tool.titleKey), style: const TextStyle(fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text(vt(context, tool.subtitleKey), style: const TextStyle(fontSize: 12, color: Colors.white60))]),
        ]),
      ),
    ),
  );
...
class _ToolStudioPageState extends State<ToolStudioPage> {
  ...
  @override
  void dispose() { prompt.dispose(); super.dispose(); }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(title), actions: [Padding(padding: const EdgeInsets.only(right: 12), child: Center(child: _CreditBadge(credits: widget.credits)))]),
      body: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 900), child: ListView(padding: const EdgeInsets.all(20), children: [
        Container(
          ...
          child: Row(children: [Container(width: 52, height: 52, decoration: BoxDecoration(gradient: LinearGradient(colors: widget.tool.colors), borderRadius: BorderRadius.circular(16)), child: Icon(widget.tool.icon, color: Colors.white)), const SizedBox(width: 14), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text(vt(context, widget.tool.subtitleKey), style: const TextStyle(color: Colors.white60))]))]),
        ),
        const SizedBox(height: 18),
        TextField(
          key: const Key('generation_prompt'), controller: prompt, minLines: 6, maxLines: 12,
        ...
        if (widget.tool.isVideo) ...[
          const SizedBox(height: 18), Text('${vt(context, 'duration')}: $seconds ${vt(context, 'seconds')}', style: const TextStyle(fontWeight: FontWeight.w800)),
        ...
        if (statusText != null) ...[const SizedBox(height: 16), Card(child: Padding(padding: const EdgeInsets.all(16), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.info_outline, color: CreatorTheme.cyan), const SizedBox(width: 10), Expanded(child: Text(statusText!))])))],
        if (job != null) ...[
          const SizedBox(height: 10),
          Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(vt(context, 'job'), style: const TextStyle(fontWeight: FontWeight.w900)), const SizedBox(height: 8),
            Text('ID: ${job!['id']}'), Text('${vt(context, 'status')}: ${job!['status']}'), Text('${vt(context, 'provider')}: ${job!['provider']}'), Text('${vt(context, 'cost')}: ${job!['cost']} ${vt(context, 'credits')}'),
            const SizedBox(height: 10), FilledButton.tonalIcon(onPressed: () => Navigator.of(context).pop(job), icon: const Icon(Icons.folder_outlined), label: Text(vt(context, 'add_projects'))),
          ]))),
        ],
      ]))),
    );
...
class AllToolsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(20), children: [
    Text(vt(context, 'all_tools'), style: const TextStyle(fontSize: 27, fontWeight: FontWeight.w900)), const SizedBox(height: 8),
    Text(vt(context, 'all_tools_sub'), style: const TextStyle(color: Colors.white60)), const SizedBox(height: 18),
    ...tools.map((tool) => Card(child: ListTile(leading: Icon(tool.icon), title: Text(vt(context, tool.titleKey), style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text(vt(context, tool.subtitleKey)), trailing: const Icon(Icons.chevron_right), onTap: () => onOpenTool(tool)))),
  ]);
}
...
class _ProjectsPageState extends State<ProjectsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(20), children: [
      Text(vt(context, 'projects'), style: const TextStyle(fontSize: 27, fontWeight: FontWeight.w900)), const SizedBox(height: 14),
      for (var i = 0; i < widget.jobs.length; i++) Card(child: ListTile(leading: const Icon(Icons.auto_awesome), title: Text('${widget.jobs[i]['type'] ?? 'AI'} • ${widget.jobs[i]['status'] ?? 'unknown'}', style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('${widget.jobs[i]['prompt'] ?? ''}', maxLines: 2, overflow: TextOverflow.ellipsis), trailing: IconButton(icon: const Icon(Icons.refresh), onPressed: () => _refreshJob(i)))),
    ]);
  }
}
...
class ProfilePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(20), children: [
      Text(vt(context, 'profile_title'), style: const TextStyle(fontSize: 27, fontWeight: FontWeight.w900)), const SizedBox(height: 18),
      Card(child: ListTile(leading: const CircleAvatar(child: Icon(Icons.person_outline)), title: Text(vt(context, 'test_user')), subtitle: Text('$credits ${vt(context, 'credits')} • ${backendOnline ? vt(context, 'cloud_on') : vt(context, 'cloud_off')}'))),
      const SizedBox(height: 12),
      ...
      Card(child: ListTile(leading: const Icon(Icons.workspace_premium_outlined), title: const Text('Veyra Pro'), subtitle: Text(vt(context, 'pro_account_sub')), trailing: const Icon(Icons.chevron_right))),
      Card(child: ListTile(leading: const Icon(Icons.shield_outlined), title: Text(vt(context, 'privacy')), subtitle: Text(vt(context, 'privacy_sub')))),
    ]);
  }
}
...
class _CreditBadge extends StatelessWidget {
  ...
  final int credits;
  @override
  Widget build(BuildContext context) => Chip(avatar: const Icon(Icons.bolt, size: 18, color: CreatorTheme.gold), label: Text('$credits ${vt(context, 'credits')}'));
}
